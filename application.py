...
def login():
  ...
  if newusername is not None:
    if len(newusername) > 2 and len(newpassword) > 2:
    ...
    elif len(username) > 2 and len(password) > 2:
      # See if user exists.
...
def location():
    ...
    return(render_template("location.html", zipcode=zipcode, returned_zip_info=returned_zip_info, lat=lat, long=long, temp=temp ))
...
def checkin():
     ...
     if yourusername is not None:
       db.execute("INSERT INTO checkin (zipcode, username, comment) VALUES (:zipcode, :username, :comment)",
            {"zipcode": newusername, "username": yourusername, "comment": comment})
